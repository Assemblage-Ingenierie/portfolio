...
- **typographie par champ du bandeau** (`fields`) : libellé et valeur stylés **indépendamment** (gras, **couleur**, **taille `sizePt`**, **petites capitales `smallCaps`** sur la valeur — ex. Mission AI), via défauts globaux (`labelBold:false`, `valueBold:true`) + surcharges par clé `WpFieldKey` (`overrides`). Défaut : libellé « Mission AI » en rouge, valeur en noir. Couleurs = **palette Assemblage** (`ASSEMBLAGE_PALETTE`). Helper `effectiveFieldStyle(resolved, key)`. Rendu en deux `<span>` (resets `!important` anti-thème ; `font-variant` défini par span pour le small-caps) ;
