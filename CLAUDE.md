...
- **Sidebar WP en menus déroulants + vue admin/user** : `WpLayoutSidebar` rend chaque section en `<details>` (Typographie générale, Champs du bandeau, Prestation Assemblage [Dev], Espacements, Catégories, Photos). Le sous-menu « Photos additionnelles » (dans la section Photos) porte le toggle « Activée / Désactivée » de la galerie + le nombre de slots. Le compteur de slots est plafonné au **nombre de photos réellement présentes sur la fiche** (`knownPhotos.length`, borné par `WP_MAX_GALLERY_SLOTS`). Les tailles de police sont saisissables à la main (slider + champ numérique via `StepSlider`). La vue est pilotée par `useViewMode()` (toggle « Vue : Admin/User » dans la toolbar `WordpressView`, visible si `canSwitch`) : la **vue user masque** « Typographie générale », « Espacements », « Catégories », le bouton « Réinitialiser le style » et le bouton « Appliquer les paramètres par défaut WordPress ».
- **Bouton admin « Appliquer les paramètres par défaut WordPress »** : applique le preset `ASSEMBLAGE_WP_DEFAULTS` (`wpConfig.ts`) — typographie générale + champs du bandeau + espacements (liste validée 08/06/26) — **par-dessus** la config courante (`{ ...config, ...preset }`), donc les réglages photos / catégories / prestation de la fiche sont conservés. C'est un **partial** : seules les clés `typo`/`fields`/`spacing` sont écrasées. Rien n'est écrit en Airtable tant que l'utilisateur ne sauvegarde pas, et la sauvegarde ne touche que la clé `wp` (cf. *Persistance*).
