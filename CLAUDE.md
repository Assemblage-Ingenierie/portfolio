...
1. **Export WP 1** (`/api/projet/[slug]/publish`) — crée un nouveau draft. Le post apparaît systématiquement dans `/wp-admin/edit.php`. L'URL Airtable `urlWordpress` est mise à jour pour pointer sur ce dernier draft. Réponse inclut `previousUrl` pour rappeler à l'utilisateur l'ancien draft à nettoyer manuellement.
   - ⚠ **Le slug canonique est libéré avant création** (`releaseCanonicalSlug`, cf. ci-dessous) : sans ça WP suffixait le draft en `-2` / `-3` / `-4`, et ce suffixe cassait le lookup par slug de `/update-prod` et `/pole-gallery`. La réponse expose `wpSlug` + `slugSuffixed` et un `warning` si WP a quand même suffixé.
2. **Itération** — l'utilisateur peut relancer Export WP 1 autant de fois qu'il veut. Chaque export = un nouveau draft ; le draft précédent est renommé `<slug>-brouillon-<id>` pour rendre le slug canonique et doit être supprimé manuellement depuis WP admin.
3. **Mettre à jour la production** (`/api/projet/[slug]/update-prod`) — quand un draft est validé visuellement, ce bouton lit le contenu du dernier draft (URL trackée dans `urlWordpress`), recherche le post de production (`findProductionPost`, cf. ci-dessous), et fait un `POST /posts/{prodId}` pour remplacer title/content/excerpt/featured_media. Le statut publish est préservé. Si aucune version publiée n'existe (404), l'utilisateur doit publier un draft manuellement dans WP admin avant de pouvoir utiliser ce bouton. Si le draft tracké **est** déjà l'article publié (publication manuelle depuis wp-admin), la route répond 409 « rien à promouvoir » plutôt que de se copier sur elle-même.
...
- **`releaseCanonicalSlug(slug)`** (`client.ts`) — appelée en tête de `/publish`. Liste les posts vivants portant exactement le slug (`findPostsBySlug`) et **renomme les brouillons** en `<slug>-brouillon-<id>`. Un post `publish` / `future` / `private` n'est **jamais** renommé (c'est l'URL SEO de production) : dans ce cas le nouveau draft sera fatalement suffixé, et la route retourne un `warning` invitant à passer par « Mettre à jour la production ». Échec non bloquant (au pire, comportement historique).
