...
- **Bouton admin « Appliquer les paramètres par défaut WordPress »** : applique le preset `ASSEMBLAGE_WP_DEFAULTS` (`wpConfig.ts`) — typographie générale + champs du bandeau + espacements (liste validée 08/06/26 ; **Mission AI valeur en petites capitales + gras**, libellé rouge) — **par-dessus** la config courante (`{ ...config, ...preset }`), donc les réglages photos / prestation de la fiche sont conservés. C'est un **partial** : seules les clés `typo`/`fields`/`spacing` sont écrasées. Rien n'est écrit en Airtable tant que l'utilisateur ne sauvegarde pas, et la sauvegarde ne touche que la clé `wp` (cf. *Persistance*).
