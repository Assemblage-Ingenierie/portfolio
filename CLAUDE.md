...
- **disposition photos** (`photos`) — ratio couverture, pleine largeur, **choix de la photo couverture par filename** (sinon défaut Airtable), **cadrage horizontal/vertical** de la couverture (`object-position`, sliders 5 %), colonnes galerie (0–4, 0 = auto), ratio et gap galerie, **réglages par photo** (`perPhoto[filename]` : enabled + offsetX/Y) appliqués sur chaque `<img>` de la galerie, **position du bloc « Prestation Assemblage »** pour le template Dev (`prestationPosition` : `before-description` / `after-description` (défaut) / `after-photos`). `buildWpContent(projet, cover, gallery, wpConfig?)` reçoit désormais des objets `WpPhoto = { url, filename }` — le filename est préservé après upload WP pour relier chaque photo à ses réglages utilisateur.
