...
- **Table `portfolio_profiles`** dans Supabase (projet INTERNAL `hhkofvbptnrtwbazftlm`, eu-west-2) : colonnes `id, email, role ('admin'|'user'), is_approved, created_at`. Trigger `on_auth_user_created_portfolio` → `handle_new_user()` insert auto à chaque signup.
  - ⚠ **Projet Supabase mutualisé** : `hhkofvbptnrtwbazftlm` est partagé par plusieurs apps Assemblage (`aichantier`, `dao`, `portfolio`, `renta`). La table `auth.users` est donc **commune** et porte un trigger `AFTER INSERT` par app (`on_auth_user_created` / `_dao` / `_portfolio` / `renta_…`), chacun créant sa propre ligne de profil applicatif.
  - **Conséquence debug accès** : un collègue peut exister dans `auth.users` (déjà connecté via une autre app) sans ligne dans `portfolio_profiles` → bloqué en page d'attente. Diagnostic : `LEFT JOIN auth.users → portfolio_profiles` (repérer `has_profile=false`). Correctif : insérer le profil manquant en reprenant l'`id` depuis `auth.users` (conforme à la RLS `auth.uid() = id`), `is_approved=true`, `role='user'`.
...
- **Multi-selects lus par field ID** (noms de colonnes pas garantis stables → `returnFieldsByFieldId: true`) :
  ...
  - `Statut` (`fldxXNdE0uNaomeby`) — multi-valeurs pour filtrage AND, fallback sur `État avancement` (single)
    ...
    - ⚠ **`normalizeStatut` ne doit JAMAIS collapser deux options qui existent réellement dans Airtable.** Le champ porte depuis 2026 deux paires de quasi-synonymes assumées : `Terminé`/`Livré` et `En cours`/`En chantier`. `lib/utils/normalize.ts` mappait `terminé → Livré` et `en cours → En chantier`, si bien qu'une fiche marquée « Terminé » s'affichait « ● Livré » partout (header PDF, export WP, filtres). Corrigé le 07/09/26 : `MAP` ne sert plus qu'au **rattrapage de valeurs libres / historiques** absentes du champ (`chantier`, `annulé`, `livraison prévue en 2024`…), et `VALID` est le **miroir exact** des options Airtable. Le fallback d'une valeur vide reste `En étude` (427 fiches sur 845 ont le champ vide — choix assumé).
    - **Source unique du type et de la liste de filtres** : l'union `Statut` et `STATUT_FILTER_OPTIONS` dans `types/projet.ts` ; les couleurs de badge dans `lib/ui/statutColors.ts` (`STATUT_BG` / `STATUT_COLOR`, typés `Record<Statut, string>` — le compilateur refuse une option sans couleur). Les 4 pages de portfolio (home, builder, tableau, public) les **importent** ; elles en portaient chacune une copie littérale avant le 07/09/26. **Ne plus réécrire ces listes à la main.**
    - ⚠ **Conséquence UX du non-collapse** : la sémantique des filtres est **AND** et une fiche ne porte qu'une seule valeur, donc cocher `Terminé` **et** `Livré` ne renvoie **aucun** résultat. Pour lister tous les projets achevés il faut deux recherches. Le remède définitif est de migrer les données (réécrire `Livré` → `Terminé` et `En chantier` → `En cours` dans Airtable, puis supprimer les options vidées) — non fait à ce jour : 301 fiches `Livré` vs 40 `Terminé`, 18 `En chantier` vs 13 `En cours`.
...
- **Toolbar** (`ProjetToolbar`) : `← Portfolio` puis, **juste à droite**, le sélecteur **Statut fiche** avec une **pastille colorée** par statut (couleur partagée via `FICHE_STATUS_COLOR` dans `projectConfig.ts`, identique à la home `PortfolioGrid`). À droite : toggle **Vue : Admin/User** (si `canSwitch`, donc admin uniquement), **Sauvegarder la mise en page** (Str-Env/Dev), **Télécharger PDF**, et un unique lien **Edition WordPress** (→ `/projet/[slug]/wordpress`). Tout l'export WP a été **déplacé dans la page d'édition WP** (`WordpressView`) — il ne reste sur la fiche que le lien d'accès.
- **Sidebar gauche** (`components/projet/LayoutSidebar.tsx`) : uniquement affichée pour les templates **Str-Env** et **Dev**. En haut de la nav : le code affaire (titre), puis le **sélecteur Template** (admin uniquement, masqué en vue user — déplacé depuis la toolbar), puis **Éditer les champs** (lien vers `/projet/[slug]/edit`) + **Recadrer les photos** (toggle `cropEditMode`). En dessous, les sections accordion (Mise en page typographique, Photo principale, Texte description, Photos additionnelles, Certifications, [Dev] Prestation Assemblage).
- Pour les templates **Solo / Diptyque / Triptyque** : pas de sidebar — le bouton "Éditer les champs" reste dans la toolbar du haut (`ProjetToolbar`) ; le sélecteur Template n'est alors pas exposé (TEMPLATE_OPTIONS ne propose que Str-Env/Dev).
...
- **Coupure du texte (spacer) — méthode partagée** : `findSplitIndex` (cale la coupure sur l'espace le plus proche de la cible %) **et** `nudgeWordBoundary` (réglage fin : décale la coupure de N frontières de mot, ±) vivent dans `lib/pdf/templates/shared.ts` et sont **partagés** par *Description projet* (templates Str-Env `manuel.ts` + Dev `dev.ts`) et *Prestation Assemblage* (`dev.ts`). Chaque `splitDescription` local appelle ces deux helpers ; ne plus dupliquer la logique de coupure dans un template. Le curseur `%` (`textCol1Percent`/`textCol2Percent`, `col1Percent`/`col2Percent`) pose le spacer grossièrement (pas de 5 %) ; les **boutons « ◄ mot / mot ► »** (`NudgeRow` dans `LayoutSidebar`) l'affinent mot par mot via les offsets `textCol1Nudge`/`textCol2Nudge` (et `col1Nudge`/`col2Nudge` pour la Prestation). Déplacer le curseur % réinitialise le nudge associé à 0 (le spacer est reposé). Offsets absents = 0 = comportement historique.
...
- **Premier niveau (admin uniquement)** : sections typo hors bandeau — *Titre de la fiche*, *Statut*, *Description projet*, *Prestation Assemblage* (`TOP_SECTION_KEYS`).
- **Sous-menu déroulant « Bandeau »** (`<details open>`) :
  ...
  - **user** : uniquement *Cellules du bandeau* + *Cellule Programme* + les 3 espacements + *Activer/désactiver les champs* (pas de sections typo, pas de lignes horizontales).
...
- **deux templates WP** dérivés de la **Vignette pôle** (`fld1PZuYO8mz0sULA`) via `wpTemplateFor(projet.vignettePoles)` : `DEV` ⇒ **Dev**, sinon **Str-Env**. **Budget et Surface restent deux cellules distinctes** côté WP. ⚠ **L'ordre de rendu du bandeau N'EST PAS partagé** entre les deux templates (`wpFieldOrder(template)`) : `WP_FIELDS_STR_ENV` (MOA · Architecte · BET associés · Budget · Surface · Entreprise · Mission AI · Programme · Matériaux) et `WP_FIELDS_DEV` (MOA · Bailleur · Architecte · Programme · Mission AI · Budget · BET associés · Matériaux · Surface · Entreprise). En revanche, l'**éditeur « Champs du bandeau » de la sidebar est partagé** (`WP_FIELD_MENU_KEYS` — tous les `WpFieldKey`, identique pour les deux templates) : un champ stylé/masqué s'applique au template qui l'affiche. Le template Dev rend en plus le bloc « Prestation Assemblage » (seule divergence de menu : la section Prestation est Dev-only) ;
- **typographie globale** (tailles description / champs clés / pitch / titre section, interlignage) ;
- **typographie par champ du bandeau** (`fields`) : libellé et valeur stylés **indépendamment** (gras, **couleur**, **taille — `labelSizePt` pour le libellé, `sizePt` pour la valeur, réglables séparément** ; le libellé retombe sur `sizePt` puis sur `typo.fieldsSizePt` si non réglé, **petites capitales `smallCaps`** et **grandes capitales `upperCase`** sur la valeur — ex. Mission AI), via défauts globaux (`labelBold:false`, `valueBold:true`) + surcharges par clé `WpFieldKey` (`overrides`). Défaut : libellé « Mission AI » en rouge, valeur en noir. Couleurs = **palette Assemblage** (`ASSEMBLAGE_PALETTE`). Helper `effectiveFieldStyle(resolved, key)`. Rendu en deux `<span>` (resets `!important` anti-thème ; `text-transform` et `font-variant` définis par span). La clé **`programmeSecondaire`** n'est PAS une cellule autonome : sa valeur est rendue dans la cellule **Programme principal**, après le principal, séparée d'un **point médian `·`**, avec sa propre typo (gras/couleur/taille/caps) — elle apparaît dans l'UI uniquement pour exposer ces options ;
...
`DEFAULT_WP_CONFIG` est le **rendu de référence / fallback** (libellés non gras, valeurs en gras, « Mission AI » libellé rouge). À ne pas confondre avec `ASSEMBLAGE_WP_DEFAULTS` (cf. ci-dessous).

- **Persistance** : `WpConfig` vit sous la clé `wp` du `ProjectConfig` unifié (même champ Airtable « Config template manuel » que `bandeau`/`manuel`), exposé en `Projet.wpConfig`. Écrit via la route `/fields` (`wpConfig` ∈ `FICHE_ONLY_FIELDS`) qui ne remplace **que** la clé `wp` (les configs PDF `bandeau`/`manuel` sont préservées). `publish/route.ts` passe `projet.wpConfig` au builder.
- **Page de preview dédiée** : `/projet/[slug]/wordpress` (`components/projet/WordpressView` + `WpLayoutSidebar` + `WordpressPreview`). L'aperçu exécute `buildWpContent` **côté client** avec les URLs Airtable et l'injecte en `srcDoc` (pas d'export PDF, pas d'upload WP pour l'aperçu). On y accède via le lien **Edition WordPress** de `ProjetToolbar` ; « Éditer les champs » (rich-text Description projet → `/projet/[slug]/edit`) est dans la **sidebar WP**. La toolbar de cette page porte les boutons d'export : **Export WP** (publie **directement en ligne** + ajoute aux pages pôle), **Ajouter à la page pôle** (vue admin — sert de reprise si l'ajout automatique a échoué), **Voir sur le site**. Le bouton « Mettre à jour la production » a été retiré le 31/08/26.
- **Sidebar WP en menus déroulants + vue admin/user** : `WpLayoutSidebar` rend chaque section en `<details>` (Typographie générale, Champs du bandeau, Prestation Assemblage [Dev], Espacements, Photos). Pas de section « Catégories » dans l'UI (la taxonomie reste assignée à l'export depuis « Tags export WP », cf. ci-dessus). L'éditeur « Champs du bandeau » est **partagé** entre les deux templates (`WP_FIELD_MENU_KEYS`). Le sous-menu « Photos additionnelles » (dans la section Photos) porte le toggle « Activée / Désactivée » de la galerie + le nombre de slots. Le compteur de slots est plafonné au **nombre de photos réellement présentes sur la fiche** (`knownPhotos.length`, borné par `WP_MAX_GALLERY_SLOTS`). Les tailles de police sont saisissables à la main (slider + champ numérique via `StepSlider`). La vue est pilotée par `useViewMode()` (toggle « Vue : Admin/User » dans la toolbar `WordpressView`, visible si `canSwitch`) : la **vue user masque** « Typographie générale », « Espacements » et le bouton « Appliquer les paramètres par défaut WordPress ».
- **Bouton admin « Appliquer les paramètres par défaut WordPress »** : applique le preset `ASSEMBLAGE_WP_DEFAULTS` (`wpConfig.ts`) — typographie générale + champs du bandeau + espacements (liste validée 08/06/26 ; tous les champs du bandeau en **13 pt** via `fieldsSizePt` ; **Mission AI : libellé rouge `labelSizePt: 13` pt, valeur noir `sizePt: 16` pt + gras + petites capitales** — `labelSizePt` est explicite car sinon le libellé tomberait sur `sizePt=16`) — **par-dessus** la config courante (`{ ...config, ...preset }`), donc les réglages photos / prestation de la fiche sont conservés. C'est un **partial** : seules les clés `typo`/`fields`/`spacing` sont écrasées. Rien n'est écrit en Airtable tant que l'utilisateur ne sauvegarde pas, et la sauvegarde ne touche que la clé `wp` (cf. *Persistance*). Un bouton **« Voir les paramètres »** (admin, sous le bouton rouge) affiche le détail de `ASSEMBLAGE_WP_DEFAULTS` en lecture seule — il lit dynamiquement l'objet, donc reste à jour si les valeurs changent.
...
- **SEO Yoast** : à l'export, `publish/route.ts` envoie `meta` dans le payload REST — `_yoast_wpseo_focuskw` = nom du projet, `_yoast_wpseo_metadesc` = `projet.metaDescription` (champ aiText Airtable « Méta description SEO », `fldQmXMJpDY7TrbfL`, lu par field ID, généré depuis « Description projet »). Le slug Yoast = slug du post (déjà envoyé). ⚠ **Prérequis WordPress** : les meta `_yoast_*` sont protégées → l'API REST les ignore tant qu'elles ne sont pas enregistrées côté WP via `register_post_meta(..., show_in_rest:true)` (mu-plugin / Code Snippets). Sans ça, l'app envoie les valeurs mais Yoast reste vide.
...
#### Workflow de publication (depuis le 31/08/26)

**« Export WP » publie directement en ligne et ajoute à la page pôle, en une seule action.**
Il n'y a plus d'étape brouillon : l'aperçu de `/projet/[slug]/wordpress` en tient lieu.

1. **Export WP** (`/api/projet/[slug]/publish`) — upload des médias, `status: 'publish'`,
   write-back de l'URL dans Airtable, puis `addProjetToPoleGalleries()`.
2. **L'ajout aux pages pôle est NON BLOQUANT.** Si l'étape échoue, l'article **reste publié** et
   la réponse porte un `warning`. L'utilisateur réessaie avec « Ajouter à la page pôle ».
   Deux **non-erreurs** signalées explicitement (elles passaient en silence avant) :
   « Vignette pôle » vide → aucune galerie cible ; pas de photo de couverture → tuile impossible.
   ⚠ `added: false` **sans** `error` = tuile déjà présente (endpoint idempotent), ce n'est
   **pas** un échec — ne pas confondre, sinon on avertit à tort à chaque réexport.
3. **Pour remplacer un article existant** : le workflow retenu est de **supprimer l'ancien**
   dans wp-admin, puis réexporter. Sinon WordPress suffixe le slug en `-2` (cf. section
   suivante) et `slugWarning` le signale.

> ⚠ Le bouton « Mettre à jour la production » a été **retiré de l'UI** le 31/08/26 (la route
> `/update-prod` existe toujours mais n'a plus d'appelant). `releaseCanonicalSlug` ne renomme
...
WordPress **ne compte pas les versions**. À chaque insertion, `wp_unique_post_slug()` teste le `post_name` demandé contre les autres posts **vivants** du même post_type (brouillons inclus) et incrémente `-2`, `-3`… jusqu'à en trouver un libre. Les posts à la corbeille ne bloquent rien (WP leur ajoute `__trashed`) — ce sont les **brouillons d'export précédents** qui squattaient le slug. Comme chaque Export WP crée un nouveau draft portant le slug canonique, le énième export récupérait `-N`, et le post publié depuis ce draft héritait du suffixe.

- **`releaseCanonicalSlug(slug)`** (`client.ts`) — appelée en tête de `/publish`. Liste les posts vivants portant exactement le slug (`findPostsBySlug`) et **renomme les brouillons** en `<slug>-brouillon-<id>`. Un post `publish` / `future` / `private` n'est **jamais** renommé (c'est l'URL SEO de production) : dans ce cas le nouveau post sera fatalement suffixé, et la route retourne un `warning`. **Depuis la publication directe (31/08/26), le remède est de supprimer l'ancien article dans wp-admin avant de réexporter** (le bouton « Mettre à jour la production » n'existe plus). Échec non bloquant.
