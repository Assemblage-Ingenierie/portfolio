...
import { renderTableau, TABLEAU_DEFAULTS_BY_MODE, TABLEAU_FIELD_KEYS, type TableauOrientation, type TableauMode, type TableauTypo } from '@/lib/pdf/tableauTemplate';
 ...
 *
 * Tailles de police optionnelles : `ts` (titre), `hs` (en-têtes), `cs`
 * (cellules), en points. Absentes ou invalides → défauts du template ;
 * `resolveTableauTypo` borne les valeurs hors plage.
 *
...
export default async function TableauPrintPage({
  ...
  searchParams: Promise<{
    ...
    ts?: string;   // taille de police du titre (pt)
    hs?: string;   // taille de police des en-têtes de colonnes (pt)
    cs?: string;   // taille de police des cellules (pt)
  }>;
}) {
  const {
    items: rawItems, fields: rawFields, orient: rawOrient, mode: rawMode,
    cln: rawCln, clv: rawClv, rpp: rawRpp,
    ts: rawTs, hs: rawHs, cs: rawCs,
  } = await searchParams;
  ...
  // Typo : chaque param est optionnel ; une valeur non numérique est ignorée
  // (undefined), une valeur hors bornes est ramenée dans la plage par
  // resolveTableauTypo côté template.
  const num = (raw?: string) => {
    const n = raw ? Number(raw) : NaN;
    return Number.isFinite(n) ? n : undefined;
  };
  const typo: Partial<TableauTypo> = {
    titleSizePt: num(rawTs),
    headSizePt: num(rawHs),
    cellSizePt: num(rawCs),
  };
  ...
  const bundle = renderTableau({
    ...
    champLibreNom, champLibreValues, rowsPerPage, typo,
  });
